...
export function StatCard({ label, value, trend, trendUp, icon: Icon, variant = 'surface' }: StatCardProps) {
  ...
  return (
    <div className={cn(
      "relative overflow-hidden p-4 md:p-4.5 rounded-2xl transition-all duration-300 group",
      isPrimary 
      ...
      <div className="relative z-10 flex justify-between items-start mb-3">
        ...
        <div className={cn(
          ...
          <Icon className={cn("w-3.5 h-3.5", isPrimary ? "text-emerald-zenith-accent" : "text-emerald-zenith-primary")} />
        </div>
      </div>

      <div className="relative z-10">
        <h3 className={cn(
          "text-[1.75rem] md:text-[2rem] font-black tracking-tight mb-1.5",
          isPrimary ? "text-emerald-zenith-accent" : "text-emerald-zenith-text"
        )}>
          {value}
        </h3>
        <div className={cn(
          "flex items-center gap-1.5 text-[11px] md:text-xs font-bold w-fit px-2 py-1 rounded-full",
          isPrimary 
